...
export interface VoiceRequest {
  ...
  settings: Settings;
  outputPath: string;
  /** The storyboard's own authoritative sum(scene.durationSeconds) — Test Mode synthesizes
   * exactly this length; a real TTS call ignores it here and is conformed to it separately by
   * voiceConform.ts, since the actual generated speech naturally comes back a different length. */
...
export async function generateVoice(request: VoiceRequest): Promise<VoiceResult> {
  const { text, voice, settings, outputPath, targetDurationSeconds } = request;

  if (!settings.openaiApiKey) {
    log.info('Test Mode: synthesizing placeholder voice track');
    const { durationSeconds } = await generateMockVoice(outputPath, targetDurationSeconds);
